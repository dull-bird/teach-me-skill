...
function bootstrapContext(state: SessionState): string {
  ...
  const lines = [
    ...
    "- After teaching, ask one small optional feedback probe; prefer multiple-choice or true/false, occasionally short-answer, and continue if the user skips.",
    "- Valuable captures are not limited to tool names; capture reusable reasoning and hidden mechanisms.",
    "- Use: python3 ~/.openclaw/skills/teach-me/scripts/teach_me.py context|configure|assess|capture|sync|style",
  ];
  if (state.manual) {
